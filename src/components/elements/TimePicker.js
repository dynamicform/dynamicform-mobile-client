...
import {timePickerPropType} from '../../utility/propTypes';
...
export class  QTimePicker extends React.Component{
    ...
    handleOnChange(date) {
        ...
        if (!this.props.isDynamic) {
            ...
            this.state.defaultvalue = moment(value, IsNullorUndefined(this.state.format) ? 'kk:mm:ss' : this.state.format);
        } else {
            const dataPosition = this.props.dataPosition;
...
QTimePicker.propTypes = timePickerPropType;
